...
function create_fragment(ctx) {
	...
	return {
		...
		h() {
			attr(button, "class", "burger svelte-auqv0m");
			attr(ul0, "class", "ul-mobile hidden svelte-auqv0m");
			attr(div, "class", "nav-mobile svelte-auqv0m");
			attr(ul1, "class", "ul-desktop svelte-auqv0m");
			attr(nav, "class", "nav svelte-auqv0m");
		},
		m(target, anchor) {
			...
			/*ul0_binding*/ ctx[6](ul0);
			append_hydration(nav, t1);
			append_hydration(nav, ul1);
...
function instance($$self, $$props, $$invalidate) {
	...
	function onBurgerClick() {
		$$invalidate(5, isMobileNavOpen = !isMobileNavOpen);
	}

	function ul0_binding($$value) {
		binding_callbacks[$$value ? 'unshift' : 'push'](() => {
			ulmobileElement = $$value;
			($$invalidate(2, ulmobileElement), $$invalidate(5, isMobileNavOpen));
		});
	}
	...
	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*ulmobileElement, isMobileNavOpen*/ 36) {
			if (ulmobileElement) {
				if (isMobileNavOpen) {
					ulmobileElement.classList.remove('hidden');

					setTimeout(
						() => {
							$$invalidate(2, ulmobileElement.style.opacity = '1', ulmobileElement);
						},
						10
					);
				} else {
					$$invalidate(2, ulmobileElement.style.opacity = '0', ulmobileElement);

					setTimeout(
						() => {
							ulmobileElement.classList.add('hidden');
						},
						500
					);
				}
